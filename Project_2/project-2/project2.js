...
class MeshDrawer {
	...
	constructor() {
		...
		this.ambientIntensityLoc = gl.getUniformLocation(this.prog, 'ambient_intensity');
...
const meshFS = `
			...
			void main() {
				...
				vec4 baseColor = showTex ? texColor : vec4(color, 1.0); // Base color depending on texture or solid color

				if (enableLighting) {
					vec3 norm = normalize(v_normal); // Normalize the interpolated normal for the fragment
					vec3 lightDirection = normalize(lightPos); // Calculate the light direction
					float diff = max(dot(norm, -lightDirection), 0.0); // Compute the diffuse factor

					vec3 ambient = ambient_intensity * baseColor.rgb; // Calculate ambient light
					vec3 diffuse = diff * baseColor.rgb; // Calculate diffuse light

					vec3 lighting = ambient + diffuse; // Combine the two
					gl_FragColor = vec4(lighting, baseColor.a); // Apply lighting to the base color
				} else {
					// No lighting, just the base color
